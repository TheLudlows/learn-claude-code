...
SharedTodoManager(Mutex<TodoManager>) 由 Agent 持有（Arc），经
ToolContext.agent.todo_manager 下传，不再用进程级全局。
*/
...
/// Thread-safe wrapper: `Mutex<TodoManager>`。由 `Agent` 持有（Arc），不再用全局 OnceLock。
///
